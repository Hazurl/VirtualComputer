...
void startCode(MachineCode code, Ram<Ram_size>* ram, CPU* cpu) {
	udword seg_instr, seg_stack, entry_point;

	ram->clean();
	MachineCodeHelper::load_to_ram<Ram_size>(ram, &code, seg_instr, seg_stack, entry_point);
	/*
	...
	*/
	cpu->define_program(seg_instr, seg_stack, entry_point);
	cpu->start_program();
}
